...
export class InitialSchema1760000000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      "CREATE TYPE transaction_category AS ENUM ('FREIGHT', 'FUEL', 'MAINTENANCE', 'INSURANCE', 'FINANCING')",
    );
    await queryRunner.query(
      "CREATE TYPE payable_category AS ENUM ('INSURANCE', 'MAINTENANCE', 'FINANCING')",
    );
    ...
    await queryRunner.query(
      'CREATE INDEX idx_transactions_wallet_category ON transactions(wallet_id, category)',
    );
    await queryRunner.query(
      'CREATE INDEX idx_payables_wallet_status_due ON payables(wallet_id, status, due_date)',
    );
    await queryRunner.query(
      'CREATE INDEX idx_open_banking_user_synced_at ON open_banking_syncs(user_id, synced_at DESC)',
    );
  }
